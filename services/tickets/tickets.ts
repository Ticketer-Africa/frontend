...
//Remove resale tickeet
export const removeResaleTicket = async (ticketId: string): Promise<Ticket[]> => {
  try {
    const res = await axios.post("/tickets/resale/remove", { ticketId });
    toast.success(res.data.message || "Resale ticket removed successfully");
    return res.data;
  } catch (error: any) {
    const errorMessage =
